...
if (log_index==1) {
mean_EI_pres <- NULL
mean_EI_pres[which(corr_mrsos_tran_year>0)] <- mean_pr[which(corr_mrsos_tran_year>0)] - (a*log(corr_mrsos_tran_year[which(corr_mrsos_tran_year>0)])+b)
mean_EI_pres[which(corr_mrsos_tran_year<0)] <- mean_pr[which(corr_mrsos_tran_year < 0)] 
mean_EI_fut[which(corr_mrsos_tran_year_fut>0)] <- mean_pr_fut[which(corr_mrsos_tran_year_fut>0)] - 
(a*log(corr_mrsos_tran_year_fut[which(corr_mrsos_tran_year_fut>0)])+b)
mean_EI_fut[which(corr_mrsos_tran_year_fut<0)] <- mean_pr_fut[which(corr_mrsos_tran_year_fut < 0)] }
if (log_index==0) { 
mean_EI_pres <- mean_pr - (a*corr_mrsos_tran_year+b)
mean_EI_fut <- mean_pr_fut - (a*corr_mrsos_tran_year_fut+b) }
...
rm(mean_PrPET); rm(mean_pr); rm(mean_PrPET_fut); rm(mean_pr_fut); rm(pres); rm(fut); rm(corr_mrsos_tran_year_fut); rm(corr_mrsos_tran_year) }


pdf("FigureS13.pdf", width=8); par(mar=c(4,5,3,3))
#### AI-based
...
mean(frac_drylands_EI_withpr_pres_MMM), mean(frac_drylands_EI_withpr_fut_MMM))), pch=8,lwd=2, col="black")
...
print(paste("mean(frac_drylands_AI_pres_allmodels)=",mean(frac_drylands_AI_pres_allmodels)))
print(paste("mean(frac_drylands_AI_fut_allmodels)=",mean(frac_drylands_AI_fut_allmodels)))

print(paste("mean(frac_drylands_EI_pres_allmodels)=",mean(frac_drylands_EI_pres_allmodels)))
print(paste("mean(frac_drylands_EI_fut_allmodels)=",mean(frac_drylands_EI_fut_allmodels)))
