...
mean_prchange <- apply(mean_pr_year_change_2x2_allmodels[,,which(list_models_common_fut %in% list_models_common_SMstress)], c(1,2), median, na.rm=T)
...
mean_index_drylands_pres[i,j] <- mean_lai[i,j]  }  ##-  (a*log(mean_corr_mrsos_tran[i,j])+b) }
...
mean_index_drylands_fut[i,j] <- mean_lai_fut[i,j] } ##-  (a*log(mean_corr_mrsos_tran[i,j])+b)}
else if ((is.na(mean_corr_mrsos_tran_fut[i,j])==F) && (mean_corr_mrsos_tran_fut[i,j] > 0)) {
mean_index_drylands_fut[i,j]  <-  mean_lai_fut[i,j] -  (a*log(mean_corr_mrsos_tran_fut[i,j])+b) } }}
} else if (log_index==0) {
mean_index_drylands_pres <- array(NA, dim=dim(mean_lai))
for (i in 1:length(lon)){
for (j in 1:length(lat)){
...
### LAI
coord_sign_lai_year_change_2x2 <- NULL
for (i in 1:181){
for (j in 1:91){
bob <- length(which(sign(mean_lai_year_change_2x2_allmodels[i,j, which(list_models_lai_fut %in% list_models_common_SMstress)  ])==1))
bill  <- length(which(sign(mean_lai_year_change_2x2_allmodels[i,j, which(list_models_lai_fut %in% list_models_common_SMstress)  ])==-1))
if ((bob > 12*0.74) || (bill > 12*0.74) ){ coord_sign_lai_year_change_2x2 <- rbind(coord_sign_lai_year_change_2x2,c(lon[i],lat[j])) }
}}
for (i in 1:dim(coord_sign_lai_year_change_2x2)[1]){
if  (coord_sign_lai_year_change_2x2[i,1] > 180)  { coord_sign_lai_year_change_2x2[i,1]<-coord_sign_lai_year_change_2x2[i,1]-360  }}
...
###### EI
## We need to make map of EI change per model... which we haven't made yet:
mean_EI_year_pres_2x2_allmodels <- array(NA, dim=dim(mean_PrPET_year_pres_2x2_allmodels))
mean_EI_year_fut_2x2_allmodels <- array(NA, dim=dim(mean_PrPET_year_pres_2x2_allmodels))
a <- a_MMM
b <- b_MMM
...
mean_EI_year_pres_2x2_allmodels[,,m] <- mean_lai - (a*corr_mrsos_tran_year+b)
mean_EI_year_fut_2x2_allmodels[,,m] <- mean_lai_fut - (a*corr_mrsos_tran_year_fut+b)   }
mean_EI_year_change_2x2_allmodels <- mean_EI_year_fut_2x2_allmodels - mean_EI_year_pres_2x2_allmodels
coord_sign_EI_year_change_2x2 <- NULL
for (i in 1:181){
for (j in 1:91){
bob <- length(which(sign(mean_EI_year_change_2x2_allmodels[i,j, ])==1))
bill  <- length(which(sign(mean_EI_year_change_2x2_allmodels[i,j, ])==-1))
if ((bob > 12*0.74) || (bill > 12*0.74) ){ coord_sign_EI_year_change_2x2 <- rbind(coord_sign_EI_year_change_2x2,c(lon[i],lat[j])) }
}}
for (i in 1:dim(coord_sign_EI_year_change_2x2)[1]){
if  (coord_sign_EI_year_change_2x2[i,1] > 180)  { coord_sign_EI_year_change_2x2[i,1]<-coord_sign_EI_year_change_2x2[i,1]-360  }}
...
pdf("Figure2_new_withEI.pdf",  width=14, height=8)
...
#### c
image.plot(lon-180, lat[lowlat:highlat], mean_lai_change[ c( (length(lon)/2+1):length(lon),
1:(length(lon)/2)),lowlat:highlat],  zlim=c(-2,2),
col=col_custom(20)[20:1], breaks=seq(-2,2,by=0.2), xaxt="n", yaxt="n", xlab="", ylab="",axis.args=list(cex.axis=1.5), cex.main=1.9,
main=expression(bold(paste(Delta,"LAI"))))
increase_drylands <- array(0, dim=dim(mean_PrPET))
increase_drylands[which( (mean_PrPET >= 0.65) & (mean_PrPET_fut<0.65))] <- 1
increase_drylands[which( (mean_PrPET < 0.65) & (mean_PrPET_fut> 0.65))] <- -1
#contour(seq(-180,180, length.out=180), seq(lat[lowlat],lat[highlat],length.out=length(lowlat:highlat)),
...
points(coord_sign_lai_year_change_2x2,pch=4, col="black", cex=.01)
